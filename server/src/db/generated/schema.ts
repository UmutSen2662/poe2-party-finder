...
export const account = pgTable("account", {
  ...
  oauth2: text().notNull(),
  templates: jsonb(),
  hostRating: numeric("host_rating", { precision: 5, scale: 2 }).default(
    "0.00",
  ),
...
export const party = pgTable(
  ...
  (table) => [
    ...
    check(
      "party_status_check",
      sql`(status)::text = ANY ((ARRAY['Gathering'::character varying, 'Started'::character varying, 'Ended'::character varying])::text[])`,
    ),
  ],
);
...
export const badgeCategory = pgTable(
  "badge_category",
  {
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.categoryId, table.badgeId],
      name: "badge_category_pkey",
    }),
  ],
);
...
export const earns = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.badgeId, table.accountId],
      name: "earns_pkey",
    }),
  ],
);
...
export const applies = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.partyId, table.accountId],
      name: "applies_pkey",
    }),
    check(
      "applies_status_check",
      sql`(status)::text = ANY ((ARRAY['Pending'::character varying, 'Accepted'::character varying, 'Rejected'::character varying, 'Kicked'::character varying])::text[])`,
    ),
  ],
);
