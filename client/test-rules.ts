...
const customRules = {
  ...
  timestamp: {
    order: (markdown.rules.text as { order: number }).order - 1,
    match: SimpleMarkdown.inlineRegex(/^<t:(\d+)(?::([tTdDfFR]))?>/),
    parse: (capture: string[]) => ({
      timestamp: capture[1],
      format: capture[2] || "f",
    }),
    html: (node: { timestamp: number; format: string }) =>
      `<span class="d-timestamp" data-timestamp="${node.timestamp}" data-format="${node.format}">${new Date(node.timestamp * 1000).toLocaleString()}</span>`,
  },
};
